    ...
    <div className="admin-layout">
      ...
      <main className="main-content">
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50/50">
          ...
          {activePage === 'complaints' && <ComplaintsRegistry complaints={complaints} users={users} onAnalyze={setSelectedIntel} onUpdate={updateStatus} />}
...
const TacticalModal = ({ complaint, users, onClose, onUpdate }) => {
        ...
        <div className="flex-1 overflow-y-auto p-10 flex flex-col lg:flex-row gap-12 bg-white/40">
          <div className="lg:w-1/2">
            <div className="relative group">
               {photos.length > 0 ? (
                 <div className="relative w-full aspect-video rounded-[32px] overflow-hidden shadow-2xl border-[6px] border-white group-hover:scale-[1.01] transition-transform">
                   <img src={photos[photoIndex]} className="w-full h-full object-cover" alt="Visual Intel" />
                   {photos.length > 1 && (
                     <div className="absolute inset-0 flex items-center justify-between p-4 opacity-0 group-hover:opacity-100 transition-opacity">
                       <button onClick={(e) => { e.stopPropagation(); setPhotoIndex(p => Math.max(0, p - 1)); }} disabled={photoIndex === 0} className="w-10 h-10 rounded-full bg-black/50 text-white flex items-center justify-center backdrop-blur-md hover:bg-black/70 disabled:opacity-30">◀</button>
                       <button onClick={(e) => { e.stopPropagation(); setPhotoIndex(p => Math.min(photos.length - 1, p + 1)); }} disabled={photoIndex === photos.length - 1} className="w-10 h-10 rounded-full bg-black/50 text-white flex items-center justify-center backdrop-blur-md hover:bg-black/70 disabled:opacity-30">▶</button>
                     </div>
                   )}
                   {photos.length > 1 && (
                     <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-md text-white px-3 py-1 rounded-lg text-[10px] font-black tracking-widest">
            ...
            <div className="mt-8 bg-slate-950 p-8 rounded-[32px] border border-teal-500/20 shadow-[0_0_50px_rgba(20,184,166,0.1)]">
               ...
               <div className="grid grid-cols-2 gap-6 mb-8">
                  ...
                  <div className="bg-white/5 p-5 rounded-2xl border border-white/5">
                     ...
                     <span className={`text-sm font-black uppercase ${complaint.ai_sentiment === 'NEGATIVE' ? 'text-red-400' : 'text-teal-400'}`}>
                        {complaint.ai_sentiment || 'NEUTRAL'} 
                        <span className="text-[10px] ml-2 opacity-50">({complaint.ai_sentiment_score ? Math.round(complaint.ai_sentiment_score * 100) : 0}%)</span>
                     </span>
                  </div>
               </div>
          ...
          <div className="lg:w-1/2 flex flex-col gap-8">
            <div className="bg-white/80 p-8 rounded-[32px] border border-white shadow-xl">
              ...
              <p className="text-gray-800 leading-relaxed text-sm font-bold italic mb-6">"{complaint.description || "The reporter provided no textual intelligence for this incident."}"</p>
              
              <div className="grid grid-cols-2 gap-4 pt-6 border-t border-gray-100">
                 <div>
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Reporter</p>
                    <p className="text-xs font-bold text-gray-800">{users?.find(u => u.id?.toString() === complaint.user_id?.toString())?.name || complaint.reporter_name || complaint.user_name || "Anonymous Citizen"}</p>
                 </div>
                 <div>
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Date Logged</p>
                    <p className="text-xs font-bold text-gray-800">{complaint.created_at?.toDate ? complaint.created_at.toDate().toLocaleString() : (complaint.created_at?.slice?.(0,16)?.replace('T', ' ') || 'Unknown Date')}</p>
                 </div>
                 <div className="col-span-2">
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Detected District</p>
                    <p className="text-xs font-bold text-gray-800">{complaint.district || "Unknown District"}</p>
                 </div>
                 <div className="col-span-2">
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Full Location Data</p>
                    <p className="text-xs font-bold text-gray-800">{complaint.address || complaint.place || "Location coordinates unavailable"}</p>
                 </div>
                 <div className="col-span-2">
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Raw Coordinates</p>
                    <p className="text-xs font-bold text-teal-600 font-mono">{complaint.latitude}, {complaint.longitude}</p>
                 </div>
              </div>
            </div>
...
/* ══════════════════════════════════════════════════════════
   🗂️  KANBAN BOARD — Complaints by Status (Column View)
   Each column = one status. Cards show all complaint details.
   ══════════════════════════════════════════════════════════ */

const KANBAN_COLUMNS = [
  { label: "Submitted",    status: "Pending",             color: "border-amber-400",   badge: "bg-amber-100 text-amber-700",   head: "bg-amber-50"  },
  { label: "Verified",     status: "Verified",            color: "border-purple-400",  badge: "bg-purple-100 text-purple-700", head: "bg-purple-50" },
  { label: "In Progress",  status: "In Progress",         color: "border-cyan-400",    badge: "bg-cyan-100 text-cyan-700",     head: "bg-cyan-50"   },
  { label: "Resolved",     status: "Resolved",            color: "border-green-400",   badge: "bg-green-100 text-green-700",   head: "bg-green-50"  },
  { label: "Escalated",    status: "Escalated",           color: "border-red-400",     badge: "bg-red-100 text-red-700",       head: "bg-red-50"    },
];
...
const ComplaintsRegistry = ({ complaints, users, onAnalyze, onUpdate }) => {
  ...
  return (
    <div className="p-6 animate-fade" style={{minHeight: '100%'}}>
      {/* ── Toolbar ── */}
      <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
        <div>
          <h2 className="text-2xl font-black text-gray-800 tracking-tighter">Incident Registry</h2>
        ...
        <div className="flex gap-3 items-center">
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-2xl text-xs w-64 font-bold focus:ring-2 ring-teal-500/20"
            />
          </div>
          <button
            onClick={() => setViewMode(viewMode === "kanban" ? "table" : "kanban")}
            className="px-5 py-2 bg-teal-600 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl hover:scale-[1.02] transition-all shadow-md shadow-teal-600/20"
          >
      ...
      {viewMode === "kanban" ? (
        ...
        <div className="flex gap-4 overflow-x-auto pb-6" style={{minHeight: '500px'}}>
          {KANBAN_COLUMNS.map(col => {
            const cards = filtered.filter(c =>
              c.status?.toLowerCase() === col.status.toLowerCase()
            );
            return (
              <div key={col.status} className={`flex-shrink-0 w-72 flex flex-col border-t-4 ${col.color} rounded-2xl bg-white shadow-lg overflow-hidden`}>
                {/* Column Header */}
                <div className={`${col.head} p-4 flex justify-between items-center`}>
                  <span className="text-xs font-black uppercase tracking-widest text-gray-700">{col.label}</span>
                  <span className={`text-xs font-black px-3 py-0.5 rounded-full ${col.badge}`}>{cards.length}</span>
                </div>
                {/* Cards */}
                <div className="flex-1 overflow-y-auto p-3 space-y-3 max-h-[calc(100vh-260px)]">
                  {cards.length === 0 ? (
                    <div className="text-center py-10 text-gray-300 text-xs font-black uppercase">No complaints</div>
                  ) : cards.map(c => (
                    <KanbanCard key={c.id} complaint={c} users={users} colBadge={col.badge} onAnalyze={onAnalyze} onUpdate={onUpdate} />
                  ))}
                </div>
              </div>
            );
        ...
        /* ── TABLE VIEW ── */
        <div className="registry-container m-0 overflow-auto">
          <table className="elite-table">
            <thead>
              <tr>
                <th>Ref ID</th><th>Type</th><th>Reporter</th><th>District</th>
                <th>Address</th><th>Submitted</th><th>Severity</th><th>Status</th><th>Action</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(c => (
                <tr key={c.id}>
                  <td className="font-black text-teal-600 text-[10px]">#{c.id?.toString().slice(-6)}</td>
                  <td className="font-bold text-sm text-gray-700 uppercase">{c.category}</td>
                  <td className="text-gray-600 text-xs font-bold">{users?.find(u => u.id?.toString() === c.user_id?.toString())?.name || c.reporter_name || c.user_name || '—'}</td>
                  <td className="text-gray-400 text-[10px] font-black uppercase">{c.district || '—'}</td>
                  <td className="text-gray-400 text-[10px] font-bold max-w-[150px] truncate" title={c.address || c.place || '—'}>{c.address || c.place || '—'}</td>
                  <td className="text-gray-400 text-[10px] font-black">{c.created_at?.toDate ? c.created_at.toDate().toISOString().slice(0,10) : (c.created_at?.slice?.(0,10) || '—')}</td>
                  <td>
                    <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${c.severity === 'High' ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-600'}`}>
                      {c.severity || 'Medium'}
                    </span>
                  </td>
                  <td>
                  ...
                  <td>
                    <button onClick={() => onAnalyze(c)} className="p-2 hover:bg-teal-50 rounded-xl text-teal-600 transition-all active:scale-90">
                      <Eye size={16}/>
                    </button>
                  </td>
                </tr>
...
/* ── Single Kanban Card ── */
const KanbanCard = ({ complaint: c, users, colBadge, onAnalyze, onUpdate }) => {
  const nextActions = {
    "Pending":    [{ label: "Verify",      status: "Verified",     cls: "bg-purple-500" }],
    "Verified":   [{ label: "Start Work",  status: "In Progress",  cls: "bg-cyan-500"   }, { label: "Escalate", status: "Escalated", cls: "bg-red-500" }],
    "In Progress":[{ label: "Resolve",     status: "Resolved",     cls: "bg-green-500"  }, { label: "Escalate", status: "Escalated", cls: "bg-red-500" }],
    "Resolved":   [{ label: "Reopen",      status: "In Progress",  cls: "bg-cyan-500"   }],
    "Escalated":  [{ label: "Verify",      status: "Verified",     cls: "bg-purple-500" }],
  };
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex justify-between items-start mb-2">
        <span className="text-[10px] font-black text-gray-400 uppercase">#{c.id?.toString().slice(-6)}</span>
        <span className={`text-[9px] font-black px-2 py-0.5 rounded-full ${colBadge}`}>{c.status}</span>
      </div>

      {/* Category */}
      <p className="font-black text-gray-800 text-sm mb-1 truncate">{c.category || 'Unknown'}</p>

      {/* Details grid */}
      <div className="grid grid-cols-2 gap-1 text-[10px] text-gray-500 font-bold mb-2">
        <span className="truncate">👤 {users?.find(u => u.id?.toString() === c.user_id?.toString())?.name || c.reporter_name || c.user_name || '—'}</span>
        <span className="truncate">🗺 {c.district || '—'}</span>
        <span>📅 {c.created_at?.toDate ? c.created_at.toDate().toISOString().slice(0,10) : (c.created_at?.slice?.(0,10) || '—')}</span>
        <span>🏢 {c.department || 'Unassigned'}</span>
      </div>

      {/* Description */}
      <p className="text-[11px] text-gray-500 mb-3" style={{display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden'}}>
        {c.description || 'No description'}
      </p>

      {/* Photo thumb */}
      {(c.photo_urls?.[0] || c.photoUrl) && (
        <img src={c.photo_urls?.[0] || c.photoUrl} alt="evidence"
          className="w-full h-24 object-cover rounded-lg mb-3 border border-gray-100"/>
      )}
      ...
      <div className="flex gap-2 flex-wrap">
        <button
          onClick={() => onAnalyze(c)}
          className="flex-1 text-[10px] font-black uppercase py-2 rounded-xl border border-teal-200 text-teal-600 hover:bg-teal-50 transition-colors"
        >
          View Details
        </button>
        {actions.map(a => (
          <button
            key={a.status}
            onClick={() => onUpdate(c.id, a.status)}
            className={`flex-1 text-[10px] font-black uppercase py-2 rounded-xl text-white ${a.cls} hover:opacity-90 transition-opacity shadow-sm`}
          >
            {a.label}
          </button>
        ))}
