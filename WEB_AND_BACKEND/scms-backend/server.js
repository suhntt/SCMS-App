...
app.post("/user/:id/profile", upload.single("photo"), async (req, res) => {
  ...
  try {
    ...
    // 2. Update Firestore (Flexible Lookup)
    let userRef = db.collection("users").doc(userId.toString());
    const doc = await userRef.get();

    if (!doc.exists) {
        // Search by internal 'id' if the doc ID (usually UID) doesn't match
        const snap = await db.collection("users").where("id", "==", parseInt(userId)).get();
        if (snap.empty) {
            // Second fallback: check if userId itself is a UID
            const snap2 = await db.collection("users").where("firebaseUid", "==", userId).get();
            if (snap2.empty) return res.status(404).json({ success: false, message: "User not found" });
            userRef = snap2.docs[0].ref;
        } else {
    ...
    res.json({ success: true, profile_picture: photoUrl });
  } catch (err) {
...
app.post("/complaint/resolve-proof/:id", upload.single("photo"), async (req, res) => {
  ...
  try {
    ...
    try {
      const userDoc = await db.collection("users").doc(doc.data().user_id?.toString()).get();
      const token = userDoc.data()?.fcm_token;
      if (token) {
...
app.post("/complaint/verify/:id", async (req, res) => {
  ...
  try {
    ...
    if (yesVotes >= 2) {
        ...
        if (data.user_id) {
            await db.collection("users").doc(data.user_id.toString()).update({
        ...
        updateData.yes_votes = 0; // Reset
        updateData.no_votes = 0;
...
app.get("/user/:id/points", async (req, res) => {
  try {
    ...
    // Fallback: If document ID doesn't match (e.g. it's a UID), search by internal 'id' field
    if (!doc.exists && !isNaN(userId)) {
        const snap = await db.collection("users").where("id", "==", parseInt(userId)).get();
